// Wulan Memory — structured, local-first memory foundation.
// Memory is intentionally separate from model training: remembering something
// does not silently retrain a model.
...
export class WulanMemoryStore {
  ...
  add(input) {
    if (!input?.content || !input?.type) {
      throw new TypeError('Memory requires content and type');
    }
  ...
  search(query, { limit = 20 } = {}) {
    ...
    return [...this.entries.values()]
      .map(memory => {
        ...
        return { memory, score: matches / terms.length };
      })
