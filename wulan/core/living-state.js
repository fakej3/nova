...
export class WulanLivingState {
  constructor(initial = {}) {
    ...
    this.activity = initial.activity ?? null;
  ...
  decayToIdle(delay = 1200) {
    ...
    this.idleTimer = setTimeout(() => this.transition(LIVING_STATES.IDLE, {
  ...
  #notify() {
    ...
    for (const listener of this.listeners) { try { listener(snapshot); } catch {} }
  }
}
