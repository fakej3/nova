...
export function createWulanCore(){
  ...
  const core={events,capabilities,memory,learning,neural,semantic,ai,state,
    ...
    recordFeedback(input){const safeInput={...input};if(safeInput.source==='conversation'&&safeInput.outcome==='accepted'&&ai.status().lastError)safeInput.outcome='none';const record=learning.record(safeInput);neural.ingestFeedback(record);if(neural.stats().updates%25===0)neural.consolidate();events.emit(WULAN_EVENTS.LEARNING_FEEDBACK,{record});events.emit(WULAN_EVENTS.NEURAL_UPDATED,{reason:'feedback',recordId:record.id,stats:neural.stats()});return record;},
