...
(() => {
  ...
  const syncLabels=()=>{if(memoryLabel)memoryLabel.innerHTML=`WORLD <b>${core.world?.entities.size??0}</b>`;if(agentsLabel)agentsLabel.innerHTML=`AGENTS <b>${core.state.agents.size}</b>`;if(memoryHint)memoryHint.textContent=`memory · ${memoryCount()} stored`;};
  ...
  async function sendMessage(raw){
    ...
    try{
      ...
      core.recordFeedback({outcome:'accepted',context:text,candidatePreference:null,source:'conversation',confidence:.5});
    }catch(error){addMessage('wulan',localReply(text),`fallback · ${error.message}`);core.recordFeedback({outcome:'rejected',context:text,correction:'agent execution failed',source:'system',confidence:.3});}
