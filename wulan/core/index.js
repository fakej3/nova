...
export function createWulanCore(){
  ...
  const core={
    ...
    recordFeedback(input){const record=learning.record(input);neural.ingestFeedback(record);if(neural.stats().updates%25===0)neural.consolidate();events.emit(WULAN_EVENTS.LEARNING_FEEDBACK,{record});events.emit(WULAN_EVENTS.NEURAL_UPDATED,{reason:'feedback',recordId:record.id,stats:neural.stats()});return record;},
    ...
    boot(){if(state.status==='ready')return state;neural.consolidate();state.status='ready';events.emit(WULAN_EVENTS.SYSTEM_READY,{agents:[...state.agents.keys()],integrations:[...state.integrations.keys()],neural:neural.stats()});return state;}
  };
  return core;
}
