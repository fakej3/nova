...
export async function getRepoSnapshot(repo = DEFAULT_REPO) {
  const [meta, branches, commits] = await Promise.all([
    github(`/repos/${repo}`),
    github(`/repos/${repo}/branches?per_page=20`),
    github(`/repos/${repo}/commits?per_page=8`),
  ]);
  return {
    repo,
    ...
    recentCommits: commits.map(c => ({ sha: c.sha, message: c.commit.message.split('\n')[0], author: c.commit.author?.name, at: c.commit.author?.date })),
  };
}
...
export async function getRepoTree(repo = DEFAULT_REPO, branch = 'master') {
  ...
  return {
    ...
    files: (data.tree ?? []).filter(item => item.type === 'blob').map(item => ({ path: item.path, size: item.size, sha: item.sha })),
  };
}
...
export function registerGithubCapabilities(world) {
  world.registerCapability({
    ...
    description: 'Read live repository metadata, branches and recent commits.',
  ...
  world.registerCapability({
    ...
    description: 'Read the current file graph of a repository branch.',
    permissions: ['github:read'],
