...
export class CapabilityRegistry {
  ...
  register(definition) {
    ...
    const capability = { version:'1.0', description:'', permissions:[], inputSchema:null, ...definition };
    this.capabilities.set(capability.id, capability); return capability;
  }
  get(id) { return this.capabilities.get(id) ?? null; }
  list() { return [...this.capabilities.values()].map(({ execute, ...metadata }) => metadata); }
  async invoke(id,input,context={}) { const capability=this.get(id); if(!capability) throw new Error(`Unknown capability: ${id}`); return capability.execute(input,context); }
}
