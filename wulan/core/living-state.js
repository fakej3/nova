export const LIVING_STATES = Object.freeze({
  IDLE: 'idle', LISTENING: 'listening', THINKING: 'thinking', REMEMBERING: 'remembering', ACTING: 'acting', LEARNING: 'learning', ERROR: 'error'
});
...
export class WulanLocalPersistence{
 ...
 save(core){try{const payload={version:2,memories:core.memory?.list?.({limit:5000})??[],learning:core.learning?.recent?.(5000)??[],neural:core.neural?.exportState?.()??null};localStorage.setItem(this.key,JSON.stringify(payload));return true;}catch{return false;}}
 load(core){try{const raw=localStorage.getItem(this.key)||localStorage.getItem('wulan-local-v1');if(!raw)return{memories:0,learning:0,neural:false};const payload=JSON.parse(raw);let memories=0,learning=0;for(const memory of Array.isArray(payload.memories)?payload.memories:[]){try{core.remember(memory);memories+=1;}catch{}}for(const record of Array.isArray(payload.learning)?payload.learning:[]){try{if(typeof core.recordFeedback==='function')core.recordFeedback(record);else core.learning?.record(record);learning+=1;}catch{}}let neural=false;if(payload.neural&&typeof core.neural?.importState==='function'){try{core.neural.importState(payload.neural);neural=true;}catch{}}return{memories,learning,neural};}catch{return{memories:0,learning:0,neural:false};}}
}
