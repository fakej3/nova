...
export class WulanNeuralLayer{
 constructor({agents=[],learningRate=.025,storageKey='wulan-neural-layer-v2'}={}){
  this.agents=[...agents];this.learningRate=learningRate;this.storageKey=storageKey;this.neurons=new Map();this.synapses=new Map();this.biases=new Map();this.episodes=[];this.updates=0;
 ...
 learn({text,agent,outcome='accepted',reward}={}){if(!text||!agent||!this.agents.includes(agent))return null;const target=reward==null?(outcome==='accepted'||outcome==='corrected'?1:outcome==='rejected'||outcome==='failed'?-1:0):clamp(Number(reward),-1,1);const x=this.encode(text);const pass=this.forward(text),hidden=pass.hidden;const targetIndex=this.agents.indexOf(agent);const targetVector=Object.fromEntries(this.agents.map((a,i)=>[a,i===targetIndex?1:0]));
  // Cross-entropy gradient at the softmax output. A negative reward reverses the update.
 ...
 load(){try{const raw=localStorage.getItem(this.storageKey);if(!raw)return;const d=JSON.parse(raw);for(const saved of d.synapses||[]){const s=this.synapses.get(`input:${saved.from}->hidden:${saved.to}`)||this.synapses.get(`hidden:${saved.from}->output:${saved.to}`);if(s){s.weight=Number(saved.weight)||0;s.updates=Number(saved.updates)||0;}}for(const [k,v] of Object.entries(d.biases||{}))if(this.biases.has(k))this.biases.set(k,clamp(Number(v)||0,-1,1));this.episodes=Array.isArray(d.episodes)?d.episodes.slice(-1000):[];this.updates=Number(d.updates)||0;}catch{}}
}
