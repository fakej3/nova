...
export function createDefaultWulanCore(){
  ...
  for (const provider of core.ai.listProviders()) {
    world.upsertEntity({ id:`provider:${provider.id}`, name:provider.name, kind:'ai-provider', status:'available-via-gateway', metadata:{ providerId:provider.id, capabilities:provider.capabilities } });
  ...
  core.neural = neural;
  core.consolidation = consolidation;

  const originalRecordFeedback = core.recordFeedback.bind(core);
  core.recordFeedback = (input) => {
    const record = originalRecordFeedback(input);
    try {
      consolidation.learnFromFeedback({ context:input.context, outcome:input.outcome, agent:input.candidatePreference, correction:input.correction, candidatePreference:input.candidatePreference, confidence:input.confidence });
    } catch { /* learning must never break the primary task */ }
    return record;
  };
