...
export class WulanMemoryStore{
 ...
 update(id,patch){const existing=this.get(id);if(!existing)throw new Error(`Unknown memory: ${id}`);const updated={...existing,...patch,id,updatedAt:new Date().toISOString()};this.entries.set(id,updated);this.save();return updated;}
 remove(id){const removed=this.entries.delete(id);if(removed)this.save();return removed;}
 snapshot(){return{version:1,entries:[...this.entries.values()]};}
 save(){try{this.persistence?.saveSync(this.storageKey,this.snapshot());}catch{}}
 load(){try{const data=this.persistence?.loadSync(this.storageKey,null);if(!data)return;for(const entry of data.entries||[])this.entries.set(entry.id,entry);this.#trim();}catch{}}
